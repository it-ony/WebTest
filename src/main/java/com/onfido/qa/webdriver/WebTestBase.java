...
public abstract class WebTestBase {
    ...
    public void beforeMethod(Method method, ITestContext context) throws Exception {
        ...
        try {
            ...
            overwriteCapabilitiesFromAnnotation(capabilities, browserAnnotations);

            capabilities.setCapability("name", String.format("%s::%s", method.getDeclaringClass().getSimpleName(), method.getName()));

            capabilities = extendCapabilities(capabilities);
    ...
    private void overwriteCapabilitiesFromAnnotation(DesiredCapabilities capabilities, List<Browser> browserAnnotations) {
        ...
        var browserOptional = browserAnnotations.stream().filter(x -> !x.browser().isEmpty()).findFirst();
        if (browserOptional.isPresent()) {
            capabilities.setBrowserName(browserOptional.get().browser());
        }

        var versionOptional = browserAnnotations.stream().filter(x -> !x.browserVersion().isEmpty()).findFirst();
        if (versionOptional.isPresent()) {
            capabilities.setCapability(CapabilityType.BROWSER_VERSION, versionOptional.get().browserVersion());
        }
    }
    ...
    protected abstract DesiredCapabilities extendCapabilities(DesiredCapabilities capabilities);
